...
if input.len == 3:
  let author = input[1]
  let email = input[2]
  for file in ["README.md", "nimtemplate.nimble"]:
    file.writeFile file.readFile.replace("Gruruya", author)
  for file in ["rebrand.nim", "src/nimtemplate.nim", "src/nimtemplate/common.nim", "tests/test.nim", ".github/dependabot.yml", ".github/workflows/build.yml", ".github/workflows/documentation.yml", ".github/workflows/nim-run/action.yml"]:
